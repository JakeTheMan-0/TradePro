...
public class TradeProCommand implements CommandExecutor, TabCompleter {
  ...
  public TradeProCommand(TradePro pl) {
//    super(Collections.singletonList("tradeprp"));
    this.pl = pl;
  }

  @Override
  public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
    if (!sender.hasPermission("tradepro.admin")) {
    ...
    MsgUtils.send(
        ...
        new String[] {
          "&f&l<----- TradePro ----->",
          "&b/trade <player> &fSend a trade request",
          "&b/tradepro reload &fReload config files",
          "&b/tradepro force <player1> <player2> &fForce 2 players to trade"
        });
    if (pl.getTradeConfig().isSpectateEnabled())
      MsgUtils.send(sender, "&e/tradepro spectate <player(s)> &fSpectate an ongoing trade");
    return true;
  }
